...
class Site extends CI_Model
{
 ...
 public function getSellasValue($id = null) {
 ...
 public function getSellasId($productid = null,$productcode = null) {
  ...
  public function getAllSellases(){
        $this->db->order_by('name');
        $q = $this->db->get('sellas');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
 ...
 public function getSellasByID($id) {
        $q = $this->db->get_where('sellas', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getCategoryByCode($code) {
     ...
     public function getSellasByCode($code) {
        $q = $this->db->get_where('sellas', array('code' => $code), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
