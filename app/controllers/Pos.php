...
class Pos extends MY_Controller {
    ...
    function index($sid = NULL, $eid = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                         $sellasvalue = $this->site->getSellasValue($this->site->getSellasId($_POST['product_id'][$r]));
                        switch ($_POST['sellas'][$r]){
                            case 'carton':
                               
                            $finalsellasvalue = $sellasvalue;
                                break;
                             case 'halfcarton':
                                $finalsellasvalue = (int)$sellasvalue/2;
                                break;
                             case 'quartercarton':
                                $finalsellasvalue = (int)$sellasvalue/4;
                                break;
                             case 'cartonandhalf':
                                $finalsellasvalue = (int)$sellasvalue/2;
                                break;
                             case 'cartonandquarter':
                                 $finalsellasvalue = (int)$sellasvalue/4;
                                 break;
                            default:
                                 $finalsellasvalue = 1;
                ...
                $item_quantity = ($_POST['sellas'][$r] == 'cartonandhalf' || $_POST['sellas'][$r] == 'cartonandquarter') ? ($_POST['quantity'][$r] * $sellasvalue) + $finalsellasvalue : $_POST['quantity'][$r] * $finalsellasvalue;
                $item_comment = $_POST['item_comment'][$r];
                $sell_as = $_POST['sellas'][$r];
        ...
        for ($r = 0; $r < $i; $r++) {
                    ...
                    switch ($_POST['sellas'][$r]){
                            ...
                            $finalsellasvalue = (int)$sellasvalue/4;
                             ...
                             case 'cartonandhalf':
                             $sellasvalue = $this->site->getSellasValue($this->site->getSellasId($_POST['product_id'][$r]));    
                             $finalsellasvalue = (int)$sellasvalue+((int)$sellasvalue/2);
                             break;
                             case 'cartonandquarter':
                                 $sellasvalue = $this->site->getSellasValue($this->site->getSellasId($_POST['product_id'][$r]));    
                           $finalsellasvalue = (int)$sellasvalue+((int)$sellasvalue/4);
                                break;
                            default:
                           $finalsellasvalue = 1;
            ...
            $item_quantity = $_POST['quantity'][$r] * $finalsellasvalue;
            $item_comment = $_POST['item_comment'][$r];
